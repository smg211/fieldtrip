function ft_plot_ieeg(pos, val, varargin)

% FT_PLOT_IEEG visualizes spatially sparse scalar data as points, spheres, spherical clouds
% of points and optionally 2D slices through the spherical clouds. This is 
...
%   'elecshape'          = 'point' plots a single 2D point at each sensor position (see plot3)
...
%   'radius'             = scalar, maximum radius of elecshape (default = 4)
...
%
% The following inputs apply when 'elecshape' = 'cloud'
...
%
% The following options apply when 'elecshape' = 'point'
...
% The following inputs apply when elecshape = 'surf' and slice = '2d'
...
elecshape          = ft_getopt(varargin, 'elecshape', 'cloud');
    ...
    for c = 1:length(pos(:,1)) % cloud loop
      ...
      if distance < rmax(c)
        ...
        elseif strcmp(elecshape, 'cloud')
          ...
          error('if slice = "2d" then elecshape must be either "surf" or "cloud"');
        end % elecshape
